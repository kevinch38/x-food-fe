...
const MerchantService = () => {
  ...
  const fetchMerchants = async (queryParams) => {
    const { data } = await axiosInstance.get(`/api/merchants`, {
      params: queryParams,
    });
    return data;
  };
  ...
  const approveMerchant = async (request) => {
    const { data } = await axiosInstance.put(
      `/api/merchants/approve/active`,
      request
    );
    return data;
  };

  const approveInactiveMerchant = async (request) => {
    const { data } = await axiosInstance.put(
      `/api/merchants/approve/inactive`,
      request
    );
    return data;
  };
