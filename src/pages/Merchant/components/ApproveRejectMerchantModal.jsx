...
export default function ApproveRejectMerchantModal({ merchantID, action }) {
	...
	useEffect(() => {
		if (merchantID) {
			const fetchMerchantDetails = async () => {
				try {
					const result = await merchantService.fetchMerchantById(
						merchantID
					);
	...
	const handleApproveInactive = () => {
		dispatch(
			merchantAction(async () => {
				await merchantService.approveInactiveMerchant(merchantID);
						...
						<div className='d-flex justify-content-center row'>
							...
							{action === 'Reject' ? (
								<>
									...
									>
										...
										request? This process cannot be undone
									</p>
								</>
							) : (
								<>
							...
							<div className='justify-content-start text-start d-flex mt-5'>
								...
								<div>
									...
									<p>
										{merchant?.status.match(
											/WAITING_FOR_(\w+)_APPROVAL/
										)
											? merchant?.status.match(
													/WAITING_FOR_(\w+)_APPROVAL/)[1]
											: null}
									</p>
								</div>
							</div>
